...
public class Cell implements Serializable, Comparable<Cell> {
	...
	protected boolean fuses;
	protected boolean divAbnormally;
	protected boolean looksSick;
	...
	Cell(int id, Cells parentCells) {
		...
		this.tillEnd = false;
		this.fuses = false;
		this.divAbnormally = false;
		this.looksSick = false;
		this.remark = "";
	}
	...
	/**
	 * 
	 * @return the Set of daughters. If no daughters of this cells exists
	 *         returns null
	 */
	...
	/**
	 * 
	 * @return the Set of mothers. If no mothers of this cells exists returns
	 *         null
	 */
	...
	public void addDaughter(Cell daughter) {
		...
		if (this.equals(daughter)) {
			IJ.showStatus("daughter" + daughter.getId()
					+ " is the same as mother- cannot add");
			return;
		}
		// no circles are allowed- if mother and daughter share a common
		// ancestor,
		// than daughter distance from this ancestor should be larger than
		// mother distance.
		...
		for (Cell ances : commonAnces) {
			...
			if (momDist >= daughtDist) {
				IJ.showStatus("cannot add daughter: " + daughter.getId()
						+ " to mother " + this.getId() + ": will cause circles");
				return;
			}
		}
		...
		if (dframes != null) { // not a new daughter
			...
			if (lastMomFrame >= firstDaughtFrame) {
				IJ.showStatus("cannot add daughter: " + daughter.getId()
						+ " to mother " + this.getId()
						+ ": mother appears later than daughter begins");
				return;
			}
		}
	...
	public void addLocation(int frame, Roi roi) {
		// converting the ROI to polygon ROI that will not hold imagePlus that
		// screws up serialization...
		Polygon poly = roi.getPolygon();
		roi = new PolygonRoi(poly, Roi.FREEROI);

		// TODO should we conduct checks such that a mother cell cannot continue
		// in frames following daughters birth?
	...
	public boolean isDivAbnomally() {
		return divAbnormally;
	}

	public void setDivAbnormally(boolean divAbnormally) {
		this.divAbnormally = divAbnormally;
	}

	public boolean isLooksSick() {
		return looksSick;
	}

	public void setLooksSick(boolean looksSick) {
		this.looksSick = looksSick;
	}
	...
	/**
	 * Dynamically goes over the parentsCells structure to issue a name for this
	 * cell
	 * 
	...
	/*
	 * Returns the Set of Cells which are the ancestors (cells without mohter)
	 * of this cell. If this cell has no mother returns itself.
	 */
	...
	public Set<Cell> getEntireLineage() {
		...
		// go to the ancestors (those with no mother) and return all their
		// descendants
	...
	/**
	 * @return the number symbolizing the distance of this cell in the lineage
	 *         created by ances, Meaning - the length of the shortest path from
	 *         ances to this cell. Returns -1 if this cell is not a descendant
	 *         of ances; Returns 0 if this cell is the ances.
	 * 
	 */
